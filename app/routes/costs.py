...
from app.utils.dropdown_options import COST_TYPE, get_dropdown_options
from app.utils.generate_costs_pdf import generate_costs_pdf
...
def add_cost():
    ...
    if request.method == 'POST':
        ...
        # Cost type â€” required, length cap
        if not cost_type:
        ...
        # Total is always derived â€” never trusted from the form
        ...
        logging.info(
            f"User '{current_user.username}' added cost entry â€” "
...
def view_costs():
    ...
    effective_end_date = end_date_str or _today_bd().strftime('%Y-%m-%d')

    query = CostEntry.query

    # Project filter â€” only apply if a real value was selected
    if project_id:
        query = query.filter(CostEntry.project_id == project_id)

    # Category filter â€” only apply if not empty
    if cost_type:
        query = query.filter(CostEntry.cost_type == cost_type)

    # Date filters â€” protected with try/except
    if start_date_str:
        try:
...
def download_pdf():
    ...
    if effective_end_date:
        try:
            query = query.filter(CostEntry.date <= datetime.strptime(effective_end_date, '%Y-%m-%d').date())
        except ValueError:
            pass
