...
class User(UserMixin, db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint("role IN ('admin', 'operator', 'viewer')", name='check_user_role'),
    )
...
class Project(db.Model):
    ...
    costs = db.relationship('CostEntry', backref='project', lazy=True, cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("status IN ('Running', 'Completed', 'On Hold')", name='check_project_status'),
    )
...
class DropdownOption(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(BD_TZ).replace(tzinfo=None))
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("option_type IN ('project_sector', 'cost_type')", name='check_dropdown_option_type'),
    )
...
class ProjectEditLog(db.Model):
    ...
    changed_at = db.Column(db.DateTime, default=lambda: datetime.now(BD_TZ).replace(tzinfo=None), index=True)
    ...
    project_rel = db.relationship('Project', backref=db.backref('edit_logs', cascade="all, delete-orphan"))
...
class ProjectDeletedLog(db.Model):
    ...
    deleted_at = db.Column(db.DateTime, default=lambda: datetime.now(BD_TZ).replace(tzinfo=None), index=True)
